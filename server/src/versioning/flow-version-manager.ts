...
export class FlowVersionManager {
  ...
  /**
   * Increment semantic version using semver package
   */
  private incrementVersion(currentVersion: string, changeType: "major" | "minor" | "patch"): string {
    // Clean version (remove 'v' prefix if present)
    const cleanVersion = semver.clean(currentVersion) || "0.0.0";
    
    // Increment based on change type
    const newVersion = semver.inc(cleanVersion, changeType);
  ...
  compareVersions(v1: string, v2: string): number {
    return semver.compare(semver.clean(v1) || "0.0.0", semver.clean(v2) || "0.0.0");
  }
  ...
  /**
   * Get versions in range (e.g., all 1.x.x versions)
   */
  getVersionsInRange(versions: string[], range: string): string[] {
    return versions.filter(v => semver.satisfies(semver.clean(v) || "0.0.0", range));
  }

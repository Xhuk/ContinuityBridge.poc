...
- **Authentication Adapters**: External API authentication system supporting OAuth2 (client credentials, authorization code), JWT (HS256/HS512/RS256/RS512 signing and validation), and Cookie-based authentication. Adapters operate in three directions: inbound (validates requests to ContinuityBridge endpoints), outbound (provides tokens for external API calls), and bidirectional. Each adapter links to encrypted vault secrets via FK and stores configuration (header names, placement strategy, token validation settings) in plaintext. Foundation complete with schema, storage layer, and vault integration types. **Status: Foundation approved by architect - awaiting secrets-service extension, vault UI forms, REST endpoints, and adapter implementations.**
